...
`;
  ...
  var ProductVariants = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this._updateDisableSelectors(this.product);
    ...
    _updateDisableSelectors(product) {

      const selectedVariant = this.selectedVariant;
      let pro_first_option = product.options[0].name
      let valueText = selectedVariant["option1"].toLowerCase();
      // start
      var allImg = this.product.media
      var thumbnailImgOdd = ""; // 保存奇数下标的内容
      var thumbnailImgEven = ""; // 保存偶数下标的内容
      allImg.forEach((e, index) => {
        let imgAlt = e.alt;
        if (imgAlt) {
          imgAlt = e.alt.toLowerCase()
        }
        
        if (pro_first_option.indexOf('Color') > -1) {

          if(imgAlt != "" && imgAlt != null){
            
          if (imgAlt.indexOf(valueText) > -1) {
            if (index % 2 === 0) {
              // 偶数下标的内容追加到奇数下标的内容后面
              thumbnailImgOdd += `
      ...
      // 将奇数和偶数下标的内容进行拼接
      var thumbnailImg = thumbnailImgOdd + thumbnailImgEven;
      $(".product__mediaListBottom").html(thumbnailImg)
      let arrImage = $(".product__thumbnail_itemImg .thumbnail_img_h");
      if (pro_first_option.indexOf('Color') > -1) {
        for (let index = 0; index < arrImage.length; index++) {
          let imgAlt = $(".product__thumbnail_itemImg:eq(" + index + ") .thumbnail_img_h").attr("alt").toLowerCase();
           if(imgAlt != "" && imgAlt != null){
          if (imgAlt.indexOf(valueText) > -1) {
            $(".product__thumbnail_itemImg:eq(" + index + ")").show();

          } else {

            $(".product__thumbnail_itemImg:eq(" + index + ")").hide();
          }
           }
        }

        let colorV = selectedVariant["option1"].toLowerCase()
        $('.' + colorV + '_0').css('display', 'block').siblings().css('display', 'none')

      } else {
        $('.decorationImage_large:eq(0)').css('display', 'block').siblings().css('display', 'none')
      }
      if (!selectedVariant) {
        return;
      }
